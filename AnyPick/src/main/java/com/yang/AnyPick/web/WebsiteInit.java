...
public class WebsiteInit {
    ...
    private static void createWebsiteJson(){
        ...
        website.setItemSelector("div.news_tu");
        rule.setLinkRule(new Rule("a[href]","attr","href","(newsDetail_forward_[0-9]+)","http://www.thepaper.cn/$1"));
        rule.setThumbnailRule(new Rule("img[src]","attr","src","//(image[0-9].thepaper.cn/image/[0-9|/]+.jpg)","$1"));
        rule.setTitleRule(new Rule("img[src]","attr","alt"));
        website.setDetailItemSelector("div[class=news_txt standard]");
        rule.setArticleRule(new Rule("*","text"));
        rule.setImgRule(new Rule("img[src]","attr","src"));
        website.setNextPageRule(new Rule("*","html","(load_index.jsp\\?nodeids=[0-9|,]+&topCids=[0-9|,]+&pageidx=)","http://www.thepaper.cn/$1!page"));
        website.setCategory(new String[]{
                ...
                "财经",
                "http://www.thepaper.cn/channel_25951",
                "思想",
                "http://www.thepaper.cn/channel_25952",
                "问政",
                "http://www.thepaper.cn/gov_publish.jsp",
                "生活",
                "http://www.thepaper.cn/channel_25953",
        });
